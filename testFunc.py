...
class Mega:
        ...
        for foldername in paths:
            if foldername != '':
                for file in files.items():
                    #every user made file appears to have a len(a) of 2 so check for those to avoid non user hidden files
                    ##print(len(file[1]['a']))
                    if (len(file[1]['a']) == 1 and file[1]['a']['n'] == foldername):
                        print(foldername + " has been found")
    ...
    def create_folder(self, name, dest=None):
        ...
        for idx, directory_name in enumerate(dirs):
            ...
            if existing_node_id:
                ##goes here if folder exists and assigns the nodes
